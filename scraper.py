    ...
    try:
        ...
        if resp.status_code in (200, 201):
            ...
            if isinstance(items, list):
                ...
                # Debug: print first item keys to verify format
                if items:
...
def process_apify_result(item: dict) -> dict | None:
    ...
    Apify FB Marketplace scraper returns fields like:
    ...
    - primary_listing_photo: {photo_image_url} (nested)
    ...
    try:
        ...
        # ── Price ──
        # Apify returns listing_price as nested: {amount: "10.00", formatted_amount: "$10"}
        price = 0
        ...
        if isinstance(listing_price, dict):
            ...
            # Fallback for flat price field
            price_str = str(item.get("price", "0"))
            price_clean = re.sub(r"[^\d.]", "", price_str)
            price = float(price_clean) if price_clean else 0

        # $1 is common on FB Marketplace (= "message for price")
        # Keep those but skip truly free items and errors
        ...
        if isinstance(location_obj, dict):
            ...
            if city and state:
                ...
                location_text = city
            # Also check city_page for display name
        ...
        # No lat/lng from Apify, so skip distance filtering
