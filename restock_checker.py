...
def supabase_upsert(table, data):
    ...
    if resp.status_code >= 400:
        print(f"  Upsert error on {table}: {resp.status_code} {resp.text[:200]}")
    return resp
...
def supabase_insert(table, data):
    ...
    resp = requests.post(url, headers=HEADERS, json=data)
    if resp.status_code >= 400:
        print(f"  Insert error on {table}: {resp.status_code} {resp.text[:200]}")
    return resp
...
def check_target_inventory(tcin, store_location_id):
    ...
    try:
        ...
        if resp.status_code == 429:
            print("  Target rate limited. Backing off.")
            time.sleep(30)
            return None
        resp.raise_for_status()
        data = resp.json()
        ...
        if reg_price and price and price < reg_price:
            on_sale = True
        return {"quantity": quantity, "status": status, "price": price, "on_sale": on_sale}
    except requests.exceptions.Timeout:
        print(f"  Target API timeout for TCIN {tcin} at store {store_location_id}")
        return None
...
def trigger_push_notifications(restock_event_ids):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"  Push notification error: {resp.status_code} {resp.text[:200]}")
    except Exception as e:
...
def run():
    ...
    if target_products and target_stores:
        print(f"\nChecking Target inventory ({len(target_products)} products x {len(target_stores)} stores)...")
        ...
        for i, store in enumerate(target_stores):
            ...
            for tcin, result in results.items():
                ...
                inventory_updates.append({
                    ...
                    "status": new_status, "quantity": new_quantity,
                    "last_checked": now, "source": "api_poll",
                    "price": result["price"], "on_sale": result["on_sale"],
                })
                if (prev_status in ("out_of_stock", "unknown") and
                    new_status in ("in_stock", "low_stock") and new_quantity > 0):
                    print(f"      RESTOCK DETECTED: {product['name']} - {new_quantity} units!")
                    restock_events.append({
                        "store_id": store_id, "product_id": product_id,
                        "previous_quantity": prev_quantity, "new_quantity": new_quantity,
                        "source": "api_poll", "created_at": now,
                    })
            time.sleep(0.5)
        ...
        for i, product in enumerate(walmart_products):
            ...
            if result:
                for store in walmart_stores[:1]:
                    ...
                    inventory_updates.append({
                        "store_id": store_id, "product_id": product_id,
                        "status": new_status, "quantity": result["quantity"],
                        "last_checked": now, "source": "web_monitor",
                        "price": result["price"], "on_sale": result["on_sale"],
                    })
                    if prev_status in ("out_of_stock", "unknown") and new_status == "in_stock":
                        print(f"      WALMART RESTOCK: {product['name']} back online!")
                        restock_events.append({
                            "store_id": store_id, "product_id": product_id,
                            "previous_quantity": 0, "new_quantity": result["quantity"],
                            "source": "web_monitor", "created_at": now,
                        })
